                        ...
                        <% if (request.getAttribute("contenidos")!=null){
                            ...
                            for (Contenido contenido : contenidos) {
                                if (contenido instanceof Produccion && (((Produccion) contenido).getPublicado()) || (contenido instanceof Produccion &&
                                    ...
                                    out.print("<a class='produccion' href='" + contenido.getAbsoluteURL() + "'>" + contenido.getTitulo() + "</a></h2>");
                                    out.println("<div class='post-meta'>Por&nbsp;<a href='" + produccion.getUser().getAbsoluteURL() + "'>" +
                                    ...
                                    out.print("<a class='produccion more-link' href='"+ contenido.getAbsoluteURL() + "'>");
                                    if (produccion.getPublicado() && produccion.getUser().getId()!=logued_user.getId()) {
                                        out.println("Leer más</a></div>");
                                    } else {
                                        out.println("Modificar</a></div>");
                                    }
                                    out.println("</div>");
...
<script>
    $(document).ready(function(){
        ...
        $(".produccion").click(function(evento){
            evento.preventDefault();
            let produccion = $(this);
            $.ajax({
                cache: false,
                url: produccion.attr("href"),
                type: "GET",
                data: {},
                success: function (data) {
